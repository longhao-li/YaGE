...
YAGE_NODISCARD auto YaGE::Hash32(const void *data, size_t size, uint32_t seed) noexcept -> uint32_t {
    uint32_t h32; // No need to initialize.

    if (size >= 16) {
        const uint8_t *const bEnd  = static_cast<const uint8_t *>(data) + size;
    ...
    while (size > 0) {
        h32 += *static_cast<const uint8_t *>(data) * 0x165667B1U;
        data = static_cast<const uint8_t *>(data) + 1;
...
YAGE_NODISCARD auto YaGE::Hash64(const void *data, size_t size, uint64_t seed) noexcept -> uint64_t {
    uint64_t h64; // No need to initialize.
    ...
    if (size >= 4) {
        ...
        h64 ^= uint64_t(temp) * 0x9E3779B185EBCA87ULL;
        data = static_cast<const uint8_t *>(data) + 4;
    ...
    while (size > 0) {
        uint8_t temp = *static_cast<const uint8_t *>(data);
        data         = static_cast<const uint8_t *>(data) + 1;

        h64 ^= uint64_t(temp) * 0x27D4EB2F165667C5ULL;
